...
DATA_PATH = os.getenv("DATA_PATH", r"E:\Code Space\churn_prediction_mlops\data\processed\telecom_churn_cleaned1.csv")
...
def main(promote_best=True, metric_key="roc_auc"):
    ...
    models = [
        ("LightGBM", LGBMClassifier(n_estimators=300, learning_rate=0.05, max_depth=7, random_state=RANDOM_STATE, n_jobs=-1)),
        ("CatBoost", CatBoostClassifier(iterations=300, depth=8, learning_rate=0.05, eval_metric="AUC", verbose=False, random_seed=RANDOM_STATE)),
        ("XGBoost",  XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=7, subsample=0.8, colsample_bytree=0.8, eval_metric="logloss", random_state=RANDOM_STATE)),
    ]
    ...
    print(f"Best: {best_name} | {metric_key}={best_metrics[metric_key]:.4f} | run_id={best_run_id}")

    if promote_best:
        client = MlflowClient(tracking_uri=MLFLOW_TRACKING_URI)
        # get latest model version created by this run
        mv = next(
            mv for mv in client.search_model_versions(f"name='{REGISTERED_MODEL_NAME}'")
        ...
        client.transition_model_version_stage(
            name=REGISTERED_MODEL_NAME, version=mv.version, stage="Production", archive_existing_versions=True
        )
